...
namespace JIN {
	...
	public:
		...
		inline bool GetGlobalKey() {
			...
			static auto key_sig = (BYTE*)"\x00\x00\x00\x00\x80\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff\xff\x00\x00\x00\x00";
			static auto key_mask = "x???xx?xx????xxxxxxxxxxx";
			if (is_found == false) {
				static uint64_t Key = FindPatternExReg(key_sig, key_mask, 0x100000) + 0x70;
				printf("Key = %p\n", Key);
				if (RPM<uint64_t>(Key - 0x18) && RPM<uint64_t>(Key - 0x10))
				{
					//GlobalKey1 = RPM<uint64_t>(Key - 0x18);
					//GlobalKey2 = RPM<uint64_t>(Key - 0x10);
					//Key1 = 9ACD88D2DCC2D821, Key2 = 2E5299C500F825E3
					GlobalKey1 = 0x9ACD88D2DCC2D821;
					GlobalKey2 = 0x2E5299C500F825E3;
					printf("Key1 = %p, Key2 = %p\n", GlobalKey1, GlobalKey2);
					return true;
				}
			}
		}
